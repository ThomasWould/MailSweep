...
Select unique IDs in priority order before issuing gets. Initially reserve up to **60 get attempts for LargeMail, 25 for OldPromotions, and 15 for OldUnreadInbox**. For OldPromotions, visit positions in a deterministic base-two bisection order. This makes a bounded prefix span the observed Gmail result order instead of taking only its first IDs, including when retries leave fewer than 25 unique messages enriched. The traversal is reproducible for the same ordered IDs; it is not random or statistically representative of the mailbox. An ID shared by cohorts is fetched once and contributes to each matching cohort after validation. After these reserved passes, spend unused attempts on still-unenriched IDs in the same priority order; the combined cap remains 100 attempts, including retries. The API must not call this sample “largest messages” because Gmail list order is not a size ranking. Call the enriched results **matching previews**.
